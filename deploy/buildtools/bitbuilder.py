...
class XilinxAlveoBitBuilder(BitBuilder):
    ...
    def setup_bitstream_repo(self) -> None:
        with open("github-bitstream-repo.yaml", "r") as f:
            bitstream_config = yaml.safe_load(f)
            self.BITSTREAM_REPO = bitstream_config

    def upload_binary_file(self, local_file_path, gh_file_path):
        print(f":DEBUG: Attempting to upload {local_file_path} to {gh_file_path}")

        g = Github(self.BITSTREAM_REPO['personal_access_token'])

        repo = g.get_user().get_repo(self.BITSTREAM_REPO['repo_name'])
        all_files = []
        ...
        tries = 10
        delay = 15
        git_hash = self.get_git_revision_hash()
        msg = f"Committing files from {git_hash}"
        upload_branch = 'main'
        r = None
    ...
    def setup_mailtrap(self) -> None:
        with open("mailtrap.yaml", "r") as f:
            mailtrap_config = yaml.safe_load(f)
            self.MAILTRAP = mailtrap_config

    def send_mail(self, title: str, body: str) -> None:
        mail = mt.Mail(
            sender=mt.Address(email=f"mailtrap@{self.MAILTRAP['mailtrap_domain']}", name="FireSim"),
            to=[mt.Address(email=f"{self.MAILTRAP['user_mail']}", name="FireSimBitBuilder")],
            cc=[],
            bcc=[],
            subject=title,
            text=body)
        client = mt.MailtrapClient(token=self.MAILTRAP['mailtrap_token'])
        try:
            client.send(mail)
        except:
            rootLogger.debug("Failed to send a notification email")
    ...
    def build_bitstream(self, bypass: bool = False) -> bool:
        ...
        rootLogger.info("Uploading bitstream file")
        sha = self.upload_binary_file(f"{local_cl_dir}/{tar_name}", f"xilinx_alveo_u250/{hwdb_entry_name}.tar.gz")

        hwdb_entry = hwdb_entry_name + ":\n"
        hwdb_entry += f"    bitstream_tar: {self.BITSTREAM_REPO['url_prefix']}/xilinx_alveo_u250/{hwdb_entry_name}.tar.gz\n"
        hwdb_entry += f"    deploy_quintuplet_override: null\n"
        hwdb_entry +=  "    custom_runtime_config: null\n"
