...
class UserTopologies:
    ...
    def fireaxe_xilinx_u250_sbus_ring_noc_octarocket_tiles_from_soc_config(self) -> None:
        edges = [
            ["xilinx_u250_firesim_octarocket_sbus_ring_noc_split_2", "xilinx_u250_firesim_octarocket_sbus_ring_noc_split_0"],
            ["xilinx_u250_firesim_octarocket_sbus_ring_noc_split_2", "xilinx_u250_firesim_octarocket_sbus_ring_noc_split_1"]
        ]

        hwdb_entries = {
            "xilinx_u250_firesim_octarocket_sbus_ring_noc_split_2"    : True,
            "xilinx_u250_firesim_octarocket_sbus_ring_noc_split_0"    : False,
            "xilinx_u250_firesim_octarocket_sbus_ring_noc_split_1"    : False,
          }
        self.fireaxe_split_topology_base_config(edges, hwdb_entries)
    ...
    def fireaxe_broadwell_config(self) -> None:
        hwdb_entries = {
            "xilinx_u250_broadwell_soc"    : True,
            "xilinx_u250_broadwell_0"    : False,
            "xilinx_u250_broadwell_1"    : False,
            "xilinx_u250_broadwell_2"    : False,
            "xilinx_u250_broadwell_3"    : False,
            "xilinx_u250_broadwell_4"    : False,
            "xilinx_u250_broadwell_5"    : False,
            "xilinx_u250_broadwell_6"    : False
          }
        for (hwdb, base) in hwdb_entries.items():
            server = FireSimServerNode(hwdb, partitioned=True, base_partition=base)
    ...
    def fireaxe_dodecaboom_config(self) -> None:
        hwdb_entries = {
            "xilinx_u250_firesim_dodeca_boom_128kB_l2_soc_30MHz"     : True,
            "xilinx_u250_firesim_dodeca_boom_128kB_l2_tiles_0_30MHz" : False,
            "xilinx_u250_firesim_dodeca_boom_128kB_l2_tiles_1_30MHz" : False
          }
        for (hwdb, base) in hwdb_entries.items():
            server = FireSimServerNode(hwdb, partitioned=True, base_partition=base)
            self.roots.append(server)

    def fireaxe_two_ring_config(self, hwdb_entries) -> None:
        socs = []
        base_servers = []
        server_edges = []
        for i in range(2):
        ...
        for soc in socs:
            for i in range(len(soc) - 1):
                server_edges.append([soc[i], soc[i+1]])

        assert(len(server_edges) == 4)
        self.roots = [FireSimPipeNode() for x in range(4)]
        for (root, edge) in zip(self.roots, server_edges):
            if isinstance(root, FireSimPipeNode):
                root.add_downlink_partition(edge)

        switch = FireSimSwitchNode()
        switch.add_downlinks(base_servers)
        self.roots.append(switch)

    def fireaxe_one_cherry_one_mono_config(self, cherry_hwdb_entries, mono_hwdb_entry) -> None:
        soc = []
        base_servers = []
        soc_edges = []

        mono_server = FireSimServerNode(mono_hwdb_entry, partitioned=False, base_partition=True)
        base_servers.append(mono_server)

        # Collect full SoC components
        for (hwdb, base) in cherry_hwdb_entries.items():
            soc_part = FireSimServerNode(hwdb, partitioned=True, base_partition=base)
            soc.append(soc_part)
            if base:
                base_servers.append(soc_part)

        for i in range(len(soc)-1):
            soc_edges.append([soc[i], soc[i+1]])
        assert(len(soc_edges) == 2)

        self.roots = [FireSimPipeNode() for x in range(2)]
        for (root, edge) in zip(self.roots, soc_edges):
            if isinstance(root, FireSimPipeNode):
                root.add_downlink_partition(edge)
    ...
    def fireaxe_128kB_dodeca_boom_ring_noc_config(self) -> None:
        hwdb_entries = {
            "xilinx_u250_firesim_nic_dodeca_boom_128kB_l2_256b_sbus_soc_10MHz"     : True,
            "xilinx_u250_firesim_nic_dodeca_boom_128kB_l2_256b_sbus_tiles_0_10MHz" : False,
            "xilinx_u250_firesim_nic_dodeca_boom_128kB_l2_256b_sbus_tiles_1_10MHz" : False
        }
        mono_hwdb_entry = "xilinx_u250_firesim_nic_dodeca_rocket_128kB_l2_config_30MHz"
        self.fireaxe_one_cherry_one_mono_config(hwdb_entries, mono_hwdb_entry)

    def fireaxe_128kB_dodeca_boom_xbar_config(self) -> None:
        hwdb_entries = {
            "xilinx_u250_firesim_nic_fame5_dodeca_boom_128kB_l2_256b_sbus_xbar_soc_10MHz"     : True,
            "xilinx_u250_firesim_nic_fame5_dodeca_boom_128kB_l2_256b_sbus_xbar_tiles_0_10MHz" : False,
            "xilinx_u250_firesim_nic_fame5_dodeca_boom_128kB_l2_256b_sbus_xbar_tiles_1_10MHz" : False
        }
        mono_hwdb_entry = "xilinx_u250_firesim_nic_dodeca_rocket_128kB_l2_config_30MHz"
        self.fireaxe_one_cherry_one_mono_config(hwdb_entries, mono_hwdb_entry)
