...
from runtools.simulation_data_classes import PartitionConfig, TipTracingConfig, TracerVConfig, AutoCounterConfig, HostDebugConfig, SynthPrintConfig
...
class FireSimServerNode(FireSimNode):
    ...
    tiptracing_config: Optional[TipTracingConfig]
    ...
    def __init__(self,
            ...
            tiptracing_config: Optional[TipTracingConfig] = None,
        ...
        self.tiptracing_config = tiptracing_config
        self.job = None
...
# shmemportname = self.uplinks[0].get_global_link_id()
            ...
            for ul in self.uplinks:
                ...
                if isinstance(uplink_node, FireSimSwitchNode):
                    shmemportname = ul.get_global_link_id()
            rootLogger.info(f"self {self} id_internal {self.server_id_internal}  {cutbridge_idxs} slotno {slotno}")
        rootLogger.info(f"shmemport {shmemportname} slotno {slotno}")
        ...
        assert self.tiptracing_config is not None
    ...
    def get_sim_start_command(self, slotno: int, sudo: bool, extra_plusargs: Optional[str]) -> str:
        ...
        if self.uplinks:
            ...
            for ul in self.uplinks:
                uplink_node = ul.uplink_side
                if isinstance(uplink_node, FireSimSwitchNode):
                    node_shmemportnames.append(ul.get_global_link_id())
            all_shmemportnames = node_shmemportnames + [self.supernode_get_sibling(x).uplinks[0].get_global_link_id() for x in range(1, num_siblings)]
        rootLogger.info(f"all_shmemportnames {all_shmemportnames}")
