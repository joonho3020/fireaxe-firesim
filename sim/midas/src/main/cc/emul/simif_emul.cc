...
simif_emul_t::simif_emul_t(const TargetConfig &config,
  ...
  if (is_qsfp) {
    // lhs -> lower index
    // rhs -> higher index
    char qsfp_lhs_owned_name[256];
    char qsfp_rhs_owned_name[256];
    char qsfp_lhs_other_name[256];
    char qsfp_rhs_other_name[256];
    int lhs_fpga_idx = (fpga_idx + fpga_cnt - 1) % fpga_cnt;
    int rhs_fpga_idx = (fpga_idx + 1) % fpga_cnt;

    sprintf(qsfp_lhs_owned_name, "/qsfp_%02d_to_%02d", fpga_idx, lhs_fpga_idx);
    sprintf(qsfp_rhs_owned_name, "/qsfp_%02d_to_%02d", fpga_idx, rhs_fpga_idx);
    sprintf(qsfp_lhs_other_name, "/qsfp_%02d_to_%02d", lhs_fpga_idx, fpga_idx);
    sprintf(qsfp_rhs_other_name, "/qsfp_%02d_to_%02d", rhs_fpga_idx, fpga_idx);

    printf("qsfp_num_chans: %d\n", config.qsfp.num_channels);
    printf("fpga_idx: %d\n", fpga_idx);
    printf("lhs_fpga_idx: %d\n", lhs_fpga_idx);
    printf("rhs_fpga_idx: %d\n", rhs_fpga_idx);
    printf("qsfp_lhs_owned_name: %s\n", qsfp_lhs_owned_name);
    printf("qsfp_rhs_owned_name: %s\n", qsfp_rhs_owned_name);
    printf("qsfp_lhs_other_name: %s\n", qsfp_lhs_other_name);
    printf("qsfp_rhs_other_name: %s\n", qsfp_rhs_other_name);

    // TODO: I think this will not work for a 2-FPGA setup
    bool lhs_shmem_setup_complete = false;
    bool rhs_shmem_setup_complete = false;
    while (!lhs_shmem_setup_complete || !rhs_shmem_setup_complete) {
      lhs_shmem_setup_complete = qsfp[0]->setup_shmem(qsfp_lhs_owned_name, qsfp_lhs_other_name);
      rhs_shmem_setup_complete = qsfp[1]->setup_shmem(qsfp_rhs_owned_name, qsfp_rhs_other_name);
    }
  }
  ...
  //  printf("fpga_idx: %d\n", fpga_idx);
  //  printf("next_fpga_idx: %d\n", next_fpga_idx);
  //  printf("prev_fpga_idx: %d\n", prev_fpga_idx);
  ...
  //  int qsfp_chan = (fpga_idx == 0) ? idx : ((idx + 1) % config.qsfp.num_channels);
