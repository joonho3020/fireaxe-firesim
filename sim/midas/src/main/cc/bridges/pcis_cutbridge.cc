...
pcis_cutbridge_t::pcis_cutbridge_t(simif_t &sim,
                             StreamEngine &stream,
...
{
  std::string cutbridge_idx_arg = std::string("+cutbridgeidx") + std::to_string(dma_no) + std::string("=");
  std::string batchsize_arg = std::string("+batch-size=");
  std::string host_latinject_cycles = std::string("+host-latinject-cycles=");
  ...
  for (auto &arg : args) {
    ...
    if (arg.find(host_latinject_cycles) == 0) {
      char *str = const_cast<char *>(arg.c_str()) + host_latinject_cycles.length();
      this->HOST_LATENCY_INJECTION_CYCLES = atoi(str);
    }
  }
  ...
  this->TO_HOST_PER_TRANSACTION_BYTES   = BIGTOKEN_BYTES * to_host_dma_transactions   * this->BATCHSIZE;
  this->FROM_HOST_PER_TRANSACTION_BYTES = BIGTOKEN_BYTES * from_host_dma_transactions * this->BATCHSIZE;

  printf("BATCHSIZE: %d, TO_HOST_PER_TRANSACTION_BYTES: %d, FROM_HOST_PER_TRANSACTION_BYTES: %d\n",
      this->BATCHSIZE,
  ...
  for (int ii = 0; ii < 2; ii++) {
    ...
    if (ftruncate(shmemfd, FROM_HOST_PER_TRANSACTION_BYTES + EXTRA_BYTES) == -1) {
      printf("failed on ftruncate\n");
      exit(1);
    }
    pcie_write_buf[ii] = (char *)mmap(nullptr,
    ...
    pcie_read_buf[ii] = (char *)mmap(nullptr,
...
void pcis_cutbridge_t::init() {
  write(mmio_addrs.host_lat_inject_cycles, this->HOST_LATENCY_INJECTION_CYCLES);
  write(mmio_addrs.host_lat_inject_cycles_valid, 1);
  ...
  auto token_bytes_received = this->pull(
      stream_to_cpu_idx,
...
void pcis_cutbridge_t::tick() {
  ...
  while (true) {
...
#ifdef LOG_TO_UART
    firesplit_printf("tick %" PRIu64 " [%d] pulled %d bytes round %d\n", tick_tracker, this->bridge_idx, token_bytes_pulled, currentround);
#endif
...
#ifdef LOG_TO_UART
    firesplit_printf("tick %" PRIu64 " [%d] pcie_read_buf[%d][%d]: %d\n",  tick_tracker, this->bridge_idx, currentround, TO_HOST_PER_TRANSACTION_BYTES, pcie_read_buf[currentround][TO_HOST_PER_TRANSACTION_BYTES]);
    firesplit_printf("tick %" PRIu64 " [%d] pcie_write_buf[%d][%d]: %d\n", tick_tracker, this->bridge_idx, currentround, TO_HOST_PER_TRANSACTION_BYTES, pcie_write_buf[currentround][TO_HOST_PER_TRANSACTION_BYTES]);
#endif
#ifdef PRINT_OUTPUT_TOKENS
    for (int ii = 0; ii < (TO_HOST_PER_TRANSACTION_BYTES/BIGTOKEN_BYTES); ii++) {
      uint8_t* out_token_uint8 = (uint8_t*)(pcie_read_buf[currentround] + ii*BIGTOKEN_BYTES);
    ...
    volatile uint8_t *polladdr = (uint8_t*)(pcie_write_buf[currentround] + FROM_HOST_PER_TRANSACTION_BYTES);
    while (*polladdr == 0) {
      ;
    }

#ifdef LOG_TO_UART
    firesplit_printf("tick %" PRIu64 " round %d polladdr != 0\n", tick_tracker, currentround);
#endif
#ifdef PRINT_INPUT_TOKENS
    for (int ii = 0; ii < (FROM_HOST_PER_TRANSACTION_BYTES/BIGTOKEN_BYTES); ii++) {
      uint8_t* in_token_uint8 = (uint8_t*)(pcie_write_buf[currentround] + ii*BIGTOKEN_BYTES);
