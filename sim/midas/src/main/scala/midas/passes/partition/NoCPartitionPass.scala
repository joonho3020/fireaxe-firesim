...
trait NoCGroupModulesToPartitionPass {
  ...
  def groupModulesToPartition(
    ...
    val replaceInstanceWithWrapper = module.body.map ((stmt: Statement) => stmt match {
      ...
      case s@Connect(_, WSubField(WRef(li), lref, _, _), DoPrim(op, Seq(WSubField(WRef(ri), rref, _, _)), consts, tpe)) =>
        if (instanceNames.contains(li._1) && instanceNames.contains(ri._1)) { // connections btw modules to wrapp that has some primops in btw
          intraPartConns.append(Connect(NoInfo, WSubField(WRef(li._1), lref), DoPrim(op, Seq(WSubField(WRef(ri._1), rref)), consts, tpe)))
          EmptyStmt
        } else {
          s
        }
...
class NoCPartitionRoutersPass 
    extends Transform
    with DependencyAPIMigration  
    with NoCGroupModulesToPartitionPass
{
  import PartitionNoCRouters._
    ...
    stmt match {
      case s@Connect(_, WSubField(WRef(li), lref, _, _), WSubField(WRef(ri), rref, rt, _)) =>
        if (ri._1 == partWrapperInst) {
          ...
          val rt = ReferenceTarget(circuitMain,
                                   NoCModule,
                                   Seq(),
                                   rref,
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val (newNoCBody, wrapperModule, _, wrapperPortToRouter) = groupModulesToPartition(partitionRoutersInstKeys, moduleDefs(nocInstKey.OfModule))
    ...
    val wrapperParentAnno = FirrtlPartWrapperParentAnnotation(InstanceTarget(
      state.circuit.main,
    ...
    newNoCBody.foreach { stmt => 
      checkInterRouterConns(
        portToNeighborAnnos,
        routerToGroupIdx,
        wrapperPortToRouter,
        state.circuit.main)(stmt)
    }
...
class NoCReparentRouterGroupPass
    extends Transform
    with DependencyAPIMigration {
  import PartitionNoCRouters._
...
class RemoveDirectWireConnectionPass extends Transform with DependencyAPIMigration  {
  ...
  private def onModule(m: Module): Module = {
    ...
    val removeWires = m.body.map ( (stmt: Statement) => stmt match {
...
class NoCCollectModulesInPathAndRegroupPass
    extends NoCReparentRouterGroupPass
    with NoCGroupModulesToPartitionPass {
  import PartitionNoCRouters._
...
// fixers.foreach { fixer => println(s"Found fixer ${fixer._1.value} of ${fixer._2.value}") }
    ...
    if (fixers.size > 0) {
      ...
      val replaceFixer = curModule.body.mapStmt ( stmt => stmt match {
        ...
        case Connect(_, lexpr, DoPrim(op, Seq(WSubField(WRef(ri), rref, rt, _)), consts, tpe)) if (ri._1 == fixerInst) =>
          val fixerIdx = portToIdxMap(rref)
          val fixerName = getFixerModuleName(fixer.name, fixerIdx)
    ...
    def onStmt(stmt: Statement): Unit = {
      stmt match {
        ...
        case s@DefNode(_, name, expr) =>
          expr.foreachExpr(childexpr => childexpr match {
...
// }
  ...
  private def findGroupAndWrap(state: CircuitState): CircuitState = {
    ...
    val hartidSourceNames = childInstances.filter(_._1.value.contains(tileHartIdNexusNodeInst)).map(_._1.value)
    val extraAvoidInstances = hartidSourceNames.toSeq ++ intSourceNames.toSeq :+ "fixedClockNode"
    ...
    val transformedAnnos = state.annotations.map {
      ...
      case a@FirrtlPortToNeighborRouterIdxAnno(rt, _, _) =>
        ...
        a.copy(target = newRt)
      case a@FirrtlEnableModelMultiThreadingAnnotation(it) if (reachableInsts.contains(it.name)) =>
        a.copy(it.copy(ofModule = wrapperModule.name))
      case anno => anno
    }
    val transformedCircuit = state.circuit.copy(modules = transformedModules)
    state.copy(circuit = transformedCircuit, annotations = transformedAnnos)
    ...
    with NoCGroupModulesToPartitionPass
{
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val dedupedWrapperPorts =
      ...
      Seq(Port(NoInfo, "clock", firrtl.ir.Input, ClockType),
          Port(NoInfo, "reset", firrtl.ir.Input, ResetType))
    ...
    val transformedAnnos = state.annotations.map ( anno => anno match {
      case a@FirrtlPortToNeighborRouterIdxAnno(rt, _, _) =>
...
class NoCConnectInterruptsPass extends Transform with DependencyAPIMigration {
  ...
  protected def execute(state: CircuitState): CircuitState = {
    connectInterrupts(state)
  }

  protected def connectInterrupts(state: CircuitState): CircuitState = {
    val igraph = InstanceKeyGraph(state.circuit)
    val wrapperInstKeyPath = igraph.findInstancesInHierarchy(partWrapperInst)
    val curModuleInstKey = wrapperInstKeyPath.head.dropRight(1).last
    val childInstances = igraph.getChildInstanceMap(curModuleInstKey.OfModule)
    ...
    val intPassThroughConnections = passThroughIntPortsToAdd.zipWithIndex.map { case (ports, idx) =>
      val inport = ports._1
...
class NoCSplitHartIdNexusNodePass extends Transform with DependencyAPIMigration {
  import PartitionNoCRouters._


  def genHartIdModule(idx: Int, tileCnt: Int): Module = {
    val pn = "hartid"
    val hartidWidth = IntWidth(BigInt(log2Ceil(tileCnt + 1)))
    val port = Port(NoInfo, pn, firrtl.ir.Output, firrtl.ir.UIntType(hartidWidth))
    val body = Block(Seq(
      DefWire(NoInfo, "x", firrtl.ir.UIntType(hartidWidth)),
      DefNode(NoInfo, "id", firrtl.ir.UIntLiteral(BigInt(idx), hartidWidth)),
      Connect(NoInfo, WRef("x"), WRef("id")),
      Connect(NoInfo, WRef(pn), WRef("x"))))
    Module(NoInfo, s"hartidNode_${idx}", Seq(port), body)
  }
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val digiTopModule = moduleDefs(digiTopInstKey.OfModule)

    val tilePRCIs = digiTopChildInsts.filter { inst =>
      inst._1.value.contains("tile_prci_domain")
    }
    val numTiles = tilePRCIs.size

    val hartIdSplitMods = new mutable.ArrayBuffer[Module]()

    var idx = 0
    val hartidSplitDigiTopBody = digiTopModule.body.map((stmt: Statement) => stmt match {
      case Connect(_, WSubField(WRef(li), lref, _,_), WSubField(WRef(ri), rref, _,_)) if (ri._1 == tileHartIdNexusNodeInst) =>
        // HACK... just increment hartidx by 1 every time we meet a tile-hartid connection
        val hartIdMod = genHartIdModule(idx, numTiles)
        hartIdSplitMods.append(hartIdMod)

        val iname = s"hartid_${idx}"
        val pname = hartIdMod.ports.head.name
        idx += 1
        Block(Seq(
          DefInstance(NoInfo, iname, hartIdMod.name),
          Connect(NoInfo, WSubField(WRef(li._1), lref), WSubField(WRef(iname), pname))
        ))
    ...
    val newMods = state.circuit.modules.flatMap {
      case m: Module if (m.name == DigitalTop) =>
        Seq(m.copy(body = hartidSplitDigiTopBody)) ++ hartIdSplitMods.toSeq
      case m => Seq(m)
    }
